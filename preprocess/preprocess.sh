#######################################################
# Things you need to modify. 要在IMavatar/process目录下运行。CUDA_VISIBLE_DEVICES=3 bash preprocess.sh
subject_name='subject4'
path='/home/xietong/IMavatar/preprocess'
video_folder=$path/$subject_name
video_names='1810.mp4  1811.mp4  1812.mp4  1814.mp4'
shape_video='1810.mp4'
fps=25
# Center crop
...
########################################################
pwd=$(pwd)
path_modnet=$(pwd)'/submodules/MODNet/MODNet'
path_deca=$(pwd)'/submodules/DECA'
path_parser=$(pwd)'/submodules/face-parsing.PyTorch/face-parsing.PyTorch'
########################################################
set -e
...
do
  ...
  python demos/demo_reconstruct.py -i $video_folder/$subject_name/"${array[0]}"/image --savefolder $video_folder/$subject_name/"${array[0]}"/"deca" --saveDepth True --saveObj True --saveCode True --saveVis False --sample_step 1  --render_orig False
done
